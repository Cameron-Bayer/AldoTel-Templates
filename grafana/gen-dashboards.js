...
function k8sOverview() {
  ...
  p.push(row('Host & workload resource usage', 11));
...
// ===========================================================================
// 4. Operations Center — one pane combining top signals from all three
// ===========================================================================
function execSummary() {
  ...
  p.push(row('Resource utilization', 23));
  ...
  // --- Logs ---------------------------------------------------------------
  p.push(row('Application logs', 29));
  ...
  // --- Attention table ----------------------------------------------------
  p.push(row('Service health details', 41));
  ...
  p.push(row('Recent cluster events', 51));
  const EVT = `ScopeName LIKE '%k8sobjectsreceiver%' AND ${TF}`;
  const EJ = (p2) => `JSONExtractString(Body, 'object', ${p2})`;
  ...
  return dashboard('clickstack-exec-summary', 'ClickStack - Operations Center',
...
// cluster health -> node/host health -> storage -> network -> capacity planning.
function hostOverview() {
  ...
  // --- Node / host health --------------------------------------------------
  p.push(row('Node health (hosts)', 20));
  ...
  p.push(timeseries('Disk operations by host (per chart interval)', { h: 8, w: 12, x: 0, y: 60 },
    ...
    { unit: 'short', desc: 'Read plus write operations completed by each host during each chart interval — the IOPS signal. This is a count per interval, not per second; interval width changes with the time range.' }));
  ...
  p.push(timeseries('Packets dropped by host (per chart interval)', { h: 8, w: 8, x: 8, y: 77 },
    ...
    { unit: 'short', desc: 'Network packets discarded by each host during each chart interval. Any sustained non-zero value points at buffer exhaustion, NIC saturation, or a misconfigured interface.' }));
  p.push(timeseries('Network interface errors by host (per chart interval)', { h: 8, w: 8, x: 16, y: 77 },
    ...
    { unit: 'short', desc: 'Interface-level receive and transmit errors per chart interval. Non-zero values usually indicate a physical problem — cabling, transceiver, or switch port — rather than an application fault.' }));

  // --- Capacity planning ---------------------------------------------------
  p.push(row('Capacity planning', 85));
  ...
  p.push(stat('Total free memory', { h: 5, w: 6, x: 18, y: 86 },
    ...
    { unit: 'decbytes', decimals: 1, desc: 'Sum of the latest free-memory reading across selected hosts — the memory left for new workloads fleet-wide. Note this is a sum, so a large number can still hide one nearly-full host; check the per-host memory chart before relying on it.' }));
