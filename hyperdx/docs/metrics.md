...
WITH cpu AS (
       ...
       SELECT greatest(0, round(100 - coalesce(cpu.value, 0) * 20
                                      ...
                                      - (1 - coalesce(ready.value, 1)) * 40, 2)) AS "Health score"
...
WITH cpu AS (
       ...
       SELECT 'Compute' AS Resource, concat(toString(round(cpu.value * 100, 2)), '% used') AS Current
       FROM cpu
       UNION ALL SELECT 'Memory', concat(toString(round(mem.value * 100, 2)), '% used') FROM mem
       UNION ALL SELECT 'Storage', concat(toString(round(disk.value * 100, 2)), '% max used') FROM disk
       UNION ALL SELECT 'Kubernetes', concat(toString(round(nodes.value * 100, 2)), '% nodes ready') FROM nodes
```
...
WITH c AS (
       ...
       SELECT c.host AS Host,
              ...
              concat(toString(round(c.cpu * 100, 2)), '%') AS CPU,
              concat(toString(round(m.mem * 100, 2)), '%') AS Memory,
...
SELECT host AS Host, mountpoint AS Filesystem,
              concat(toString(round(used / nullIf(total, 0) * 100, 2)), '%') AS "Used",
...
WITH u AS (
       ...
       SELECT u.node AS Node, round(u.used, 2) AS "Used cores", c.cores AS "Observed cores",
              concat(toString(round(u.used / nullIf(c.cores, 0) * 100, 2)), '%') AS Utilization
...
WITH p AS (
       ...
       SELECT node AS Node, formatReadableSize(used) AS Used,
              ...
              concat(toString(round(used / nullIf(used + available, 0) * 100, 2)), '%') AS Utilization
...
SELECT ResourceAttributes['k8s.node.name'] AS Node,
              concat(toString(round(argMaxIf(Value, TimeUnix, MetricName = 'k8s.node.filesystem.usage') /
                                    nullIf(argMaxIf(Value, TimeUnix, MetricName = 'k8s.node.filesystem.capacity'), 0) * 100, 2)), '%') AS "Filesystem used"
       FROM default.otel_metrics_gauge
       WHERE TimeUnix > now() - INTERVAL 1 HOUR
         AND MetricName IN ('k8s.node.filesystem.usage', 'k8s.node.filesystem.capacity')
               GROUP BY Node ORDER BY argMaxIf(Value, TimeUnix, MetricName = 'k8s.node.filesystem.usage') / nullIf(argMaxIf(Value, TimeUnix, MetricName = 'k8s.node.filesystem.capacity'), 0) DESC
```
...
WITH p AS (
       ...
       SELECT p.ns AS Namespace, p.pod AS Pod,
              ...
              if(isNaN(p.cpu_lim), '-', concat(toString(round(p.cpu_lim * 100, 2)), '%')) AS "CPU/limit",
              if(isNaN(p.mem_lim), '-', concat(toString(round(p.mem_lim * 100, 2)), '%')) AS "Mem/limit",
...
SELECT Host, Volume,
              ...
              round(100 * sumIf(value, state = 'used') / nullIf(sum(value), 0), 2) AS "Used %"
       FROM (
         SELECT ResourceAttributes['host.name'] AS Host,
         ...
         GROUP BY Host, Volume, state
               ...
               GROUP BY Host, Volume ORDER BY sumIf(value, state = 'used') / nullIf(sum(value), 0) DESC
```
...
WITH host_cpu AS (
       ...
       SELECT Resource, Type, concat(toString(round(utilization_ratio * 100, 2)), '%') AS Utilization,
